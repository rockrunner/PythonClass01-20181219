...
class WoniuSalesThree:
    ...
    def test_login(self):
        conn = http.client.HTTPConnection(host=self.host, port=self.port)#建立链接
        param = 'username=admin&password=admin123&verifycode=0000'#正文部分
        ...
        header = {'Content-Type': 'application/x-www-form-urlencoded'}#头部字段
    ...
    def test_customer_home(self):
        ...
        conn.request(method='GET', url='/customer', headers=header)
        resp = conn.getresponse()
    ...
    def test_customer_add(self):
        ...
        conn.request(method='POST', url='/customer/add', body=data.encode(), headers=header)
        resp = conn.getresponse()
        print(resp.read().decode())
        query_result = Support().query_one("select customerphone from customer where customerphone='%s%s'" % ('138', customer_phone))
    ...
    def test_customer_search(self):
        ...
        conn.request(method='POST', url='/customer/search', body=data, headers=header)
        resp = conn.getresponse()
        content = resp.read().decode()
        print(type(content))
        print(content)
